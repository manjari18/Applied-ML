...
def answer_two():
    ...
    benigns = np.where(cancerdf['target']==1.0)
    data = [np.size(malignants),np.size(benigns)]
    series = pd.Series(data,index=indexes)
    #nswer = cancerdf.target.value_counts(ascending=True) # Your code here
    #nswer.index = "malignant benign".split()
...
def answer_three():
    ...
    X = cancerdf.drop('target',axis=1)# Your code here
    y = cancerdf['target']
    #print(type(X))
    #print(type(y))
    return X, y
...
def answer_five():
    ...
    knn.fit(X_train,y_train)# Your code here
...
def answer_six():
    ...
    knn = answer_five() # Your code here
...
def answer_seven():
    ...
    y_predict = knn.predict(X_test)# Your code here
    
    return y_predict # Return your answer

#answer_seven()
#np.size(answer_seven())
...
def answer_eight():
    ...
    predict_score = knn.score(X_test,y_test) # Your code here
    
    return predict_score # Return your answer
